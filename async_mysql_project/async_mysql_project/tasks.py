...
def generate_excel(sid, data):
    ...
    try:
        ...
        # Создание строки с названиями колонок как значениями
        empty_rows_1_dict = dict.fromkeys(columns_list)
        empty_rows_2_dict = dict.fromkeys(columns_list)
        for col in columns_list:
            empty_rows_1_dict[col] = col.replace('Дата оплаты ', '')
            if not 'Дата оплаты' in col and not 'Сумма оплаты' in col and not 'Color' in col:
                empty_rows_2_dict[col] = ""
            else:
                empty_rows_2_dict[col] = col

        empty_rows_1 = pd.DataFrame([empty_rows_1_dict])
        empty_rows_2 = pd.DataFrame([empty_rows_2_dict])

...
def link_generation(request, mode, service=None, row_id=None):
...
async def skeleton(request, user,
    ...
    options = [
        ...
        ('status', 'Статус'),
        ('way', 'Способ закупки'),
        ('initiator', 'Инициатор закупки (ИТ/АХО)'),
        ('KTSSR', 'КЦСР'),
        ('KOSGU', 'КОСГУ'),
        ('DopFC', 'ДопФК'),
        ('NMCC', 'НМЦК'),
        ...
        ('registration_number', 'Реестровый номер контракта (ЕИС)'),
        ('contract_number', 'Номер контракта'),
        ('contract_date', 'Дата контракта'),
        ('end_date', 'Окончание даты исполнения'),
        ('contract_price', 'Цена контракта'),
        ('execution_contract_plan', 'Исполнение контракта (план) (формула)'),
        ('remainder_old_year', 'Остаток контракта с предыдущего года'),

        # План по месяцам
        ('january_one', 'Январь (план)'),
        ('february', 'Февраль (план)'),
        ('march', 'Март (план)'),
        ('april', 'Апрель (план)'),
        ('may', 'Май (план)'),
        ('june', 'Июнь (план)'),
        ('july', 'Июль (план)'),
        ('august', 'Август (план)'),
        ('september', 'Сентябрь (план)'),
        ('october', 'Октябрь (план)'),
        ('november', 'Ноябрь (план)'),
        ('december', 'Декабрь (план)'),
        ('january_two', 'Остаток контракта на следующий год'),

        # Факт по контракту
        ('execution_contract_fact', 'Исполнение контракта (факт) (формула)'),
        ('paid_last_year', 'Оплачено в предыдущем году'),

        # Оплаты с датами
        ('date_january_one', 'Дата оплаты Январь (факт 1)'),
        ('sum_january_one', 'Сумма оплаты Январь (факт 1)'),
        ('date_february', 'Дата оплаты Февраль (факт)'),
        ('sum_february', 'Сумма оплаты Февраль (факт)'),
        ('date_march', 'Дата оплаты Март (факт)'),
        ('sum_march', 'Сумма оплаты Март (факт)'),
        ('date_april', 'Дата оплаты Апрель (факт)'),
        ('sum_april', 'Сумма оплаты Апрель (факт)'),
        ('date_may', 'Дата оплаты Май (факт)'),
        ('sum_may', 'Сумма оплаты Май (факт)'),
        ('date_june', 'Дата оплаты Июнь (факт)'),
        ('sum_june', 'Сумма оплаты Июнь (факт)'),
        ('date_july', 'Дата оплаты Июль (факт)'),
        ('sum_july', 'Сумма оплаты Июль (факт)'),
        ('date_august', 'Дата оплаты Август (факт)'),
        ('sum_august', 'Сумма оплаты Август (факт)'),
        ('date_september', 'Дата оплаты Сентябрь (факт)'),
        ('sum_september', 'Сумма оплаты Сентябрь (факт)'),
        ('date_october', 'Дата оплаты Октябрь (факт)'),
        ('sum_october', 'Сумма оплаты Октябрь (факт)'),
        ('date_november', 'Дата оплаты Ноябрь (факт)'),
        ('sum_november', 'Сумма оплаты Ноябрь (факт)'),
        ('date_december', 'Дата оплаты Декабрь (факт)'),
        ('sum_december', 'Сумма оплаты Декабрь (факт)'),
        ('date_january_two', 'Дата оплаты Январь (факт 2)'),
        ('sum_january_two', 'Сумма оплаты Январь (факт 2)'),

        # Формулы
        ('execution', '% исполнения (формула)'),
        ('contract_balance', 'Остаток по контракту (формула)')
    ]
...
async def data_table_view(request):
    """Достаём все total_pages_full во время сброса фильтров в JS"""
    ...
    SSR = link_generation(request, True)
...
async def add(request):
    ...
    context = link_generation(request, False)

    context['total_pages'] = request.GET.get('total_pages', '1')
...
async def copy(request, row_id):
    ...
    context = link_generation(request, False, service=service, row_id=row_id)

    context['page'] = request.GET.get('total_pages', '1')
...
async def edit_user(request, row_id):
    ...
    context = link_generation(request, False, service=service_user, row_id=row_id)
...
async def edit_user_two(request, row_id):
    ...
    context = link_generation(request, False, service=service_user_two, row_id=row_id)
